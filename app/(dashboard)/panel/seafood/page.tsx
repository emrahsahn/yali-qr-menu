...
import { Fish, Thermometer, Sparkles, Scale } from "lucide-react"
import { VenueSubNav } from "@/components/dashboard/venue-sub-nav"
import { useLanguage } from "@/lib/context/language-context"

export default function SeafoodPanelPage() {
  const { t } = useLanguage()
  const seafoodColor = "var(--venue-seafood)"

  const placeholderSections = [
    {
      title: t("panel.seafood.dailyShowcase"),
      desc: t("panel.seafood.dailyShowcaseDesc"),
      icon: Fish,
      count: t("panel.seafood.dailyShowcaseCount")
    },
    {
      title: t("panel.seafood.weightOrders"),
      desc: t("panel.seafood.weightOrdersDesc"),
      icon: Scale,
      count: t("panel.seafood.weightOrdersCount")
    },
    {
      title: t("panel.seafood.coldStorage"),
      desc: t("panel.seafood.coldStorageDesc"),
      icon: Thermometer,
      count: t("panel.seafood.coldStorageCount")
    }
  ]
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-6 text-left">
        <div>
          <h1 className="font-heading font-black text-2xl md:text-3xl text-foreground flex items-center gap-2.5">
            <Fish className="h-7 w-7" style={{ color: seafoodColor }} />
            <span>{t("panel.seafood.title")}</span>
          </h1>
          <p className="text-xs text-foreground/60 leading-relaxed font-semibold mt-1">
            {t("panel.seafood.subtitle")}
          </p>
        </div>
            ...
            <div key={idx} className="glass-panel p-6 rounded-3xl border-white/20 dark:border-black/10 text-left flex flex-col gap-4">
              <div className="flex justify-between items-start">
                <div
                  ...
                  style={{
                    borderColor: seafoodColor,
                    backgroundColor: "rgba(40,160,180,0.1)",
                    color: seafoodColor
                  }}
                >
                  <Icon className="h-5 w-5" />
                </div>
                <span className="text-xs font-bold text-foreground bg-white/5 px-2.5 py-1 rounded-xl border border-white/10 max-w-[150px] truncate text-center">
                  {sec.count}
                </span>
              </div>
